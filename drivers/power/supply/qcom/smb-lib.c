				...
				void *data, int disable, const char *client)
{
	...
	#if (defined(CONFIG_KERNEL_CUSTOM_WAYNE) || defined (CONFIG_KERNEL_CUSTOM_WHYRED) || defined (CONFIG_KERNEL_CUSTOM_TULIP)|| defined (CONFIG_KERNEL_CUSTOM_JASMINE))
	disable = 0;
	#endif
	if (disable) {
		/*
...
#elif defined(CONFIG_KERNEL_CUSTOM_WAYNE)|| defined (CONFIG_KERNEL_CUSTOM_JASMINE)
	if ((lct_backlight_off) && (LctIsInCall == 0) && (val->intval > 2)) {
		    return 0;
	}
#else
...
#if defined(CONFIG_KERNEL_CUSTOM_WAYNE)|| defined (CONFIG_KERNEL_CUSTOM_JASMINE)
